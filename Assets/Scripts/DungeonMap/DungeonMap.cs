...
public class DungeonMap : SingleTon<DungeonMap>
{
    ...
    private void GetRoomClicked()
    {
        ...
        foreach (Collider2D collider2D in collider2DArray)
        {
            if (collider2D.GetComponent<InstantiatedRoom>() != null)
            {
                ...
                if (instantiatedRoom.room.isClearedOfEnemies && instantiatedRoom.room.isPreviousVisited)
                {
    ...
    private IEnumerator MovePlayerToRoom(Vector3 worldPosition, Room room)
    {
        ...
        Vector3 spawnPosition = HelpUtilities.GetSpawnPositionNearestToPlayer(worldPosition);
